...
static ssize_t led_read(struct file *filp, char __user *user_buf, size_t count, loff_t *f_pos); // Declaration for read function
...
static struct file_operations led_fops = {
    ...
    .release = led_release,
    .write = led_write,
    .read = led_read, // Add read function
};
...
static ssize_t led_write(struct file *filp, const char __user *user_buf,
    ...
    spin_lock(&led_lock);
    if (value == '1' && led_state == 1) {
        spin_unlock(&led_lock);
        printk(KERN_INFO "LED is already on\n");
        return count;
    } else if (value == '0' && led_state == 0) {
        spin_unlock(&led_lock);
        printk(KERN_INFO "LED is already off\n");
        return count;
    }
...
static ssize_t led_read(struct file *filp, char __user *user_buf, size_t count, loff_t *f_pos) {
    char value = (led_state == 1) ? '1' : '0'; // Convert state to '0'/'1'
...
static int __init led_driver_init(void) {
    ...
    ret = alloc_chrdev_region(&led_dev, LED_DEV_MINOR, 1, "my_led_dev");
    if (ret < 0) {
        return ret;
    }
