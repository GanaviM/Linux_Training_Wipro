...
#define DEVICE_NAME "struct_add_device"
...
static ssize_t my_device_write(struct file *file, const char __user *buffer, size_t len, loff_t *ppos) {
    ...
    if (copy_from_user(&nums, buffer, sizeof(struct Numbers))) {
        return -EFAULT;
    }
    ...
    memcpy(device_memory, &nums, sizeof(struct Numbers));
...
MODULE_DESCRIPTION("Example Device Driver with mmap for Adding Two Numbers");
