...
#define DEVICE_NAME "integer_addition_device"
...
static ssize_t my_device_write(struct file *file, const char __user *buffer, size_t len, loff_t *ppos) {
    int nums[10], result = 0;

    if (len < sizeof(int) * 10) {
        return -EINVAL;
    }
    ...
    memcpy(nums, device_memory, sizeof(int) * 10);

    printk(KERN_INFO "Received operation: +");
    for (int i = 0; i < 10; i++) {
        printk(KERN_INFO "num%d: %d", i + 1, nums[i]);
        result += nums[i];
    }
...
MODULE_DESCRIPTION("Example Device Driver with mmap for addition of 10 integers");
